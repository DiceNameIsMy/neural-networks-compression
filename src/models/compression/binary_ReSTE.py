...
class FnBinarize(torch.autograd.Function):
    @staticmethod
...
class FnBinarizeWithReSTE(torch.autograd.Function):
    @staticmethod
...
class BinarizeWithReSTE(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        return FnBinarizeWithReSTE.apply(
            x, self.threshold.to(x.device), self.o.to(x.device)
        )
